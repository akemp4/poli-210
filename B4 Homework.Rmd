...
Treating `X_coin()` like a random variable describe it:
  ...
  c. What is the expectation of `X_coin()`?
    Probability of 1 for 'H' is .5 and the probability of 'T' is .5
    Therefore 'X_coin' should be expected as (1*.5)+(0*.5)=.5+0= .5 (50%)
    
  d. What is the variance of `X_coin()`?
      'x_coin' variance is: (.5)*1^2+(.5)*0^2 - .5^2= .25
...
Treating `X_biased_coin()` like a random variable describe it:
  a. What is the probability of 1? .7
    Probability of getting a '1' is 70%, meaning there is a 70% chance of obtaining 'H' because it is a biased coin.
  
  b. What is the probability of 0? .3
    Probability of getting a '0' is 30%, meaning there is a 30% chance of obtaining 'T' because it is a biased coin.

5. Using math, calculate:
  a. The expectation of `X_coin()`
    E(x)=1*0.5+0*0.5=0.5
    Expectation is 50% accounting for equal probable outcomes of 0 through 1.
  
  b. The expectation of `X_biased_coin()`
    E(x)=1*0.7+0*0.3=0.7
  Expectation is 70% that is reflecting a higher chance/probability of getting a 1 due to it being a biased coin.
  
  c. The expectation of `X_coin()` + `X_biased_coin()`
    E(xsum)=E(xcoin)+E(xbiased_coin)= .5 + .7 = 1.2
    
    Expectation sum of 120%
...
for(i in 1:nsims){
  ...
  unbiased_plus_biased_coin_sims[i] <- X_coin_sims[i] + X_biased_coin_sims[i] #INSERT CODE#
}
...
```{r}
df<-data.frame(gpa_diff = gpa_treat_minus_control_sims)

ggplot(df, aes(x = gpa_diff)) +
  geom_histogram(fill = "orange", color = "black", bins = 30) +
  labs(title = "Treatment and Control GPA Differences",
       x = "Difference in GPA", y = "Frequency") +
  theme_classic()
